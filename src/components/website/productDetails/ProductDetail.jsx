...
export default function ProductDetail({ product, language }) {
  ...
  return (
    <div className="max-w-6xl mt-10 font-body mx-auto p-6 grid md:grid-cols-2 gap-14">
      {/* Image Section */}
      ...
      <div className="space-y-6 font-body">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-gray-500 text-lg">
            {product.rating} ({product.avgRate}/5) Average Reviews
          </span>
        </div>
        {/* Variant Options */}
        <div className="flex items-center space-x-3">
          {product.variants.map((variant, index) => (
            <button
              key={index}
              className={`w-[150px] h-12 rounded-xl border-2 ${
        ...
        {/* Price */}
        <div className="flex font-bold items-center space-x-2 text-lg md:text-xl">
          <p>Price:</p>
          {product.hasDiscount ? (
            <>
